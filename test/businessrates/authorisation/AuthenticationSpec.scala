...
class AuthenticationSpec extends ControllerSpec {
  ...
  "Calling the authentication endpoint when the user is not logged in to Government Gateway" must "return a 401 status and the INVALID_GATEWAY_SESSION error code" in {
    val res = testController.authenticate()(FakeRequest())
    status(res) mustBe UNAUTHORIZED
    contentAsJson(res) mustBe Json.obj("errorCode" -> "INVALID_GATEWAY_SESSION")
  }

  "Calling the authentication endpoint when the user is logged in to Government Gateway but has not registered a CCA account" must "return a 401 status and the NO_CUSTOMER_RECORD error code" in {
  ...
  "Calling the authentication endpoint when the user is logged in to Government Gateway and has registered a CCA account" must "return a 200 status and the organisation ID and person ID" in {
