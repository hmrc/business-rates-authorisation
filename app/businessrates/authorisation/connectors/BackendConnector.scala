...
class BackendConnector @Inject()(@Named("voaBackendWSHttp") val http: WSHttp,
  ...
  override def getAssessment(organisationId: Long, authorisationId: Long, assessmentRef: Long, role: PermissionType)
    ...
    getLink(organisationId, authorisationId) map {
      case Some(PropertyLink(_, _, _, _, _, pending @ true, _, _, _)) =>
        None
      case Some(PropertyLink(_, _, owner @ `organisationId`, _, _, _, assessments, _, _)) =>
        assessments.find(_.assessmentRef == assessmentRef)
      case Some(PropertyLink(_, _, _, _, _, _, assessments, agents, _)) =>
        agents.find(_.organisationId == organisationId).map {
          case Party(permissions, `RepresentationApproved`, _) =>
            permissions.find { p => role == any || p.values.get(role).exists(_ != NotPermitted) }
        }.flatMap {
          case Some(_) => assessments.find(_.assessmentRef == assessmentRef)
          case _ => None
        }
      case _ => None
    }
