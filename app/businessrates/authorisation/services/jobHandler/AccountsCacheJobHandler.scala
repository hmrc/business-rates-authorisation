...
package businessrates.authorisation.services.jobHandler
...
@Singleton
class AccountsCacheJobHandler @Inject()(accountsCache: AccountsMongoCache)(implicit ec: ExecutionContext) {
  ...
  def processJob(): Future[Unit] =
    for {
      ...
      if (updatedCreatedAtTimestamp > 0) {
        ...
        log.info("No accountsCache records updated")
      }
    }
}
